...
function buildStitchList(layers, widthIn, heightIn, opts) {
  ...
  rast.masks.forEach((entry, idx) => {
    ...
    const madeira = nearestMadeira(entry.hex, (opts && opts.madeiraCatalog) || "rayon");
    colorStops.push({
      hex: (madeira && madeira.hex) || entry.hex,
      name: (madeira && madeira.name) || entry.name,
      index: idx,
      madeiraCode: madeira && madeira.code,
      madeiraBrand: madeira && madeira.brand,
      sourceHex: entry.hex,
    });
