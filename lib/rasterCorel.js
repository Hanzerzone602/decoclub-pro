 ...
 * OPT-IN only (engine=raster-corel|invent-warp|hallucinate). Default Vectorize stays bezier.
 */
...
/** Soft SRC that is a degraded twin of the Corel prior → enable hallucinate. */
...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  // Opt-in only (New Bot 2026-09-06): never auto-apply tiger prior on default Vectorize.
  // Explicit fuse=hallucinate|teacher|warp|corel-look OR caller already chose invent-warp engine.
  ...
  if (useHallucinate) {
    ...
    if (opts.fuse !== "recolor-only" && inventWarp.available()) {
      try {
        ...
        /* fall through to fill-recolor hallucinate */
      }
    }
...
module.exports = {
  ...
  RECIPE:
    ...
    "Opt-in only (engine=raster-corel|invent-warp|hallucinate). corel-import for Corel SVG upload.",
};
