...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  if (useHallucinate) {
    // Prefer ECC+multi-ROI TPS invent-warp when script/prior available
    let warpNote = null;
    if (opts.fuse === "recolor-only") {
      warpNote = "recolor-only";
    } else if (!inventWarp.available()) {
      warpNote = "invent-warp-unavailable→hallucinate (" + (inventWarp.unavailableReason ? inventWarp.unavailableReason() : "unavailable") + ")";
    } else {
      try {
        const w = inventWarp.vectorizeToSvg(pngBuf, widthIn, heightIn, opts);
        if (w && w.svg) return w;
        warpNote = "invent-warp-empty→hallucinate";
      } catch (e) {
        warpNote = "invent-warp-failed→hallucinate (" + String((e && e.message) || e).slice(0, 180) + ")";
      }
    }
    const packedH = hallucinate(pngBuf, widthIn, heightIn, opts);
    if (warpNote && packedH && packedH.vec) {
