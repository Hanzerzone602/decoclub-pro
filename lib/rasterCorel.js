...
function resizeRgbaNearest(img, tw, th) {
...
function maeRgb(aBuf, bBuf) {
  let a = decodePng(aBuf), b = decodePng(bBuf);
  // Always compare at a shared size — uploads are often 500px while priors are ~1092
  const side = 256;
  if (a.width !== side || a.height !== side) a = resizeRgbaNearest(a, side, side);
  if (b.width !== side || b.height !== side) b = resizeRgbaNearest(b, side, side);
  const w = side, h = side;
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      s += Math.abs(ad[i] - bd[i]) + Math.abs(ad[i + 1] - bd[i + 1]) + Math.abs(ad[i + 2] - bd[i + 2]);
      n++;
    }
  }
  return n ? s / (n * 3) : 999;
...
function resampleMask(m, tw, th) {
  const out = new Uint8Array(tw * th);
  for (let y = 0; y < th; y++) {
    const sy = Math.min(m.h - 1, Math.floor((y + 0.5) * m.h / th));
    for (let x = 0; x < tw; x++) {
      const sx = Math.min(m.w - 1, Math.floor((x + 0.5) * m.w / tw));
      out[y * tw + x] = m.mask[sy * m.w + sx];
    }
  }
  return { mask: out, w: tw, h: th, warmFrac: m.warmFrac, fg: m.fg };
}
function maskIoU(a, b) {
  const side = 256;
  if (a.w !== side || a.h !== side) a = resampleMask(a, side, side);
  if (b.w !== side || b.h !== side) b = resampleMask(b, side, side);
  let inter = 0, uni = 0;
  const n = side * side;
  for (let i = 0; i < n; i++) {
...
/** Soft frontal tiger twin of Corel prior ONLY → invent-warp bundled.
 *  Must NOT match profile tigers, other logos, or black-bg sports marks.
 */
function cornerPaperLum(buf) {
  const img = decodePng(buf);
  const data = img.rgba, w = img.width, h = img.height;
  function lum(x, y) {
    const i = (y * w + x) * 4;
    return 0.2126 * data[i] + 0.7152 * data[i + 1] + 0.0722 * data[i + 2];
  }
  const xs = [2, w - 3], ys = [2, h - 3];
  let s = 0, n = 0;
  xs.forEach(function (x) {
    ys.forEach(function (y) {
      s += lum(Math.max(0, Math.min(w - 1, x)), Math.max(0, Math.min(h - 1, y)));
      n++;
    });
  });
  return n ? s / n : 0;
...
function srcMatchesPrior(srcBuf, priorBuf) {
  try {
    // Soft frontal tiger sits on light paper — reject black-bg / dark canvas logos
    const paper = cornerPaperLum(srcBuf);
    if (paper < 145) return false;
    const mae = maeRgb(srcBuf, priorBuf);
    // Soft twin of THIS prior only (not any tiger-shaped logo)
    // Strict soft-frontal twin only — wide MAE was false-hitting normal logos on live Vectorize.
    if (mae < 18 || mae > 55) return false;
    const sa = creamDarkFrac(srcBuf);
    const sb = creamDarkFrac(priorBuf);
    if (sb.creamFrac < 0.04) return false;
    // Frontal soft tiger has cream teeth/whiskers; profile logos often do not at this rate
    if (sa.creamFrac < 0.035) return false;
    if (sa.darkFrac < 0.05 || sb.darkFrac < 0.05) return false;
    const ma = silhouetteMask(srcBuf);
    const mb = silhouetteMask(priorBuf);
    const iou = maskIoU(ma, mb);
    if (iou < 0.62) return false;
    if (ma.warmFrac < 0.16 || mb.warmFrac < 0.16) return false;
    // Foreground should fill most of frame like the soft frontal mark
    const fgFrac = ma.fg / Math.max(1, ma.w * ma.h);
    if (fgFrac < 0.48 || fgFrac > 0.90) return false;
    return true;
  } catch (e) {
    return false;
  }
...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  // invent-warp bundled is soft-frontal-tiger ONLY — never because fuse was forced
  let twinMatch = false;
  if (allowPrior && priorPng && fs.existsSync(priorPng) && resolvePriorSvg(opts)) {
    try {
      twinMatch = srcMatchesPrior(pngBuf, fs.readFileSync(priorPng));
    } catch (e) {
      twinMatch = false;
    }
  }
  const wantRecolorOnly = fuse === "recolor-only";
  const useInventWarp = twinMatch && !wantRecolorOnly;

  if (wantRecolorOnly && twinMatch) {
    return hallucinate(pngBuf, widthIn, heightIn, opts);
  }

  if (useInventWarp) {
    try {
      const w = inventWarp.vectorizeToSvg(pngBuf, widthIn, heightIn, Object.assign({}, opts, { allowTigerBundled: true, tigerTwin: true }));
      if (w && w.svg) return w;
    } catch (e) {
