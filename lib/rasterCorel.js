...
function srcMatchesPrior(srcBuf, priorBuf) {
  try {
    ...
    if (sa.creamFrac < 0.02) return false;
    ...
    if (iou < 0.52) return false;
    if (ma.warmFrac < 0.14 || mb.warmFrac < 0.14) return false;
    // Foreground should fill most of frame like the soft frontal mark
    const fgFrac = ma.fg / Math.max(1, ma.w * ma.h);
    if (fgFrac < 0.45 || fgFrac > 0.92) return false;
    return true;
  } catch (e) {
    return false;
  }
