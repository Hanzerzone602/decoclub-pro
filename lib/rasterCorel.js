...
function srcMatchesPrior(srcBuf, priorBuf) {
  try {
    ...
    if (mae < 25 || mae > 85) return false;
    const sa = creamDarkFrac(srcBuf);
    const sb = creamDarkFrac(priorBuf);
    if (sb.creamFrac < 0.05) return false;
    ...
    if (ma.warmFrac < 0.18 || mb.warmFrac < 0.18) return false;
    return true;
  } catch (e) {
    return false;
  }
...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  if (useHallucinate) {
    // invent-warp live ECC+TPS, or bundled soft-tiger Corel-look SVG — never silent mush for prior-match
    if (opts.fuse !== "recolor-only" && inventWarp.available()) {
      try {
        ...
        if (inventWarp.loadBundledWarped) {
          try {
            return inventWarp.loadBundledWarped(widthIn, heightIn, "invent-warp-failed→bundled (" + String((e && e.message) || e).slice(0, 120) + ")");
          } catch (e2) { /* fall through */ }
        }
      }
    }
    const packedH = hallucinate(pngBuf, widthIn, heightIn, opts);
    if (packedH && packedH.vec) {
      packedH.vec.meta = Object.assign({}, packedH.vec.meta || {}, {
        recipe: "hallucinate-after-invent-warp-miss",
        degradedFrom: "invent-warp",
        inventWarpAvailable: inventWarp.available(),
        inventWarpReason: inventWarp.unavailableReason && inventWarp.unavailableReason(),
      });
      packedH.meta = packedH.vec.meta;
    }
    return packedH;
  }
