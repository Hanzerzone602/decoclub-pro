...
function pythonDepsOk() {
  ...
  try {
    ...
    _depsOk = r.status === 0;
    _depsErr = _depsOk ? null : String((r.stderr || r.stdout || "python deps missing")).slice(0, 400);
  } catch (e) {
    _depsOk = false;
...
function available() {
  if (!fs.existsSync(SCRIPT) || !fs.existsSync(HELPER) || !fs.existsSync(DEFAULT_PRIOR_SVG)) {
    _depsErr = _depsErr || "script/prior missing";
    return false;
  }
  return pythonDepsOk();
}
...
function unavailableReason() {
  ...
  return _depsErr || "invent-warp unavailable";
}

function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  opts = opts || {};
  if (!available()) {
    const err = new Error("invent-warp not available: " + unavailableReason());
