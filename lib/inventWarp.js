...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  // Production default: ship the proven soft-tiger winner SVG.
  // Live ECC only when explicitly requested — live can exit 0 with a mush intermediate.
  const preferLive = opts.preferLive === true || process.env.INVENT_WARP_PREFER_LIVE === "1";
  if (!preferLive && fs.existsSync(BUNDLED_WARPED_SVG)) {
    return loadBundledWarped(widthIn, heightIn, "ecc-multiROI-TPS-bundled");
  }
  if (!liveReady()) {
    return tryBundled(unavailableReason() || "invent-warp live not ready");
  }
