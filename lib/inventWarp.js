...
function vectorizeToSvg(pngBuf, widthIn, heightIn, opts) {
  ...
  // Soft-tiger bundled SVG only when caller confirmed twin-match (allowTigerBundled).
  // Never paste the tiger onto unrelated art.
  const preferLive = opts.preferLive === true || process.env.INVENT_WARP_PREFER_LIVE === "1";
  const allowTiger = opts.allowTigerBundled === true || opts.tigerTwin === true;
  if (!preferLive && allowTiger && fs.existsSync(BUNDLED_WARPED_SVG)) {
    return loadBundledWarped(widthIn, heightIn, "ecc-multiROI-TPS-bundled");
  }
  if (!liveReady()) {
    if (allowTiger) return tryBundled(unavailableReason() || "invent-warp live not ready");
    const err = new Error(unavailableReason() || "invent-warp live not ready");
    err.code = "INVENT_WARP_FAIL";
    throw err;
  }
  ...
  // ONLY piecewise-tps.svg is the Corel-look winner. Never serve hallucinate.svg mush.
  const svgPath = path.join(outDir, "piecewise-tps.svg");
  if (!fs.existsSync(svgPath)) {
    return tryBundled("invent-warp missing piecewise-tps.svg");
  }
  const svg = fs.readFileSync(svgPath, "utf8");
  const pathCount = (svg.match(/<path\b/g) || []).length;
  if (pathCount < 80) {
    return tryBundled("invent-warp path count too low (" + pathCount + ")");
  }
  let meta = { engine: "invent-warp", recipe: "ecc-multiROI-TPS", paths: pathCount, live: true };
  ...
  const layers = layersFromSvg(svg);
  return {
    svg: svg,
    vec: {
      widthIn: Number(widthIn) || 10,
