...
const { padMask, dilate, mooreTraceFrom, traceAllContours, simplify, simplifyClosed, optimalPolygon, traceSubpixelContours } = require("./contour");
...
function majorityDownsample(rgba, w, h, maxEdge) {
  const m = Math.max(w, h);
  if (m <= maxEdge) return { rgba: rgba, w: w, h: h };
  const nw = Math.max(1, Math.round(w * (maxEdge / m)));
  const nh = Math.max(1, Math.round(h * (maxEdge / m)));
  const out = Buffer.alloc(nw * nh * 4);
  for (let y = 0; y < nh; y++) {
    const y0 = Math.floor(y * h / nh);
    const y1 = Math.max(y0 + 1, Math.floor((y + 1) * h / nh));
    for (let x = 0; x < nw; x++) {
      ...
      const x1 = Math.max(x0 + 1, Math.floor((x + 1) * w / nw));
      const hist = new Map();
      let bestK = 0, bestN = -1, best = [0, 0, 0, 255];
      for (let yy = y0; yy < y1; yy++) {
        for (let xx = x0; xx < x1; xx++) {
          const i = (yy * w + xx) * 4;
          const a = rgba[i + 3];
          if (a < 12) {
            ...
            hist.set(k, { n: n, rgb: [0, 0, 0, 0] });
            if (n > bestN) { bestN = n; bestK = k; best = [0, 0, 0, 0]; }
            continue;
          }
          const k = qkey(rgba[i], rgba[i + 1], rgba[i + 2]);
          let rec = hist.get(k);
          ...
          if (rec.n > bestN) {
            bestN = rec.n;
            bestK = k;
            best = [Math.round(rec.r / rec.n), Math.round(rec.g / rec.n), Math.round(rec.b / rec.n), 255];
          }
        }
      }
      const di = (y * nw + x) * 4;
      out[di] = best[0]; out[di + 1] = best[1]; out[di + 2] = best[2]; out[di + 3] = best[3];
    }
  }
  return { rgba: out, w: nw, h: nh };
}
...
function popularityPalette(rgba, w, h, k) {
  ...
  for (let i = 0; i < bins.length && colors.length < k; i++) {
    ...
    for (let j = 0; j < colors.length; j++) {
      if (labDist2(lab, rgbToLab(colors[j][0], colors[j][1], colors[j][2])) < 16) { near = true; break; }
    }
...
function snapToPalette(rgba, w, h, palette) {
  ...
  for (let i = 0; i < w * h; i++) {
    ...
    assign[i] = bi;
    counts[bi]++;
    rgba[i * 4] = Math.round(palette[bi][0]);
    rgba[i * 4 + 1] = Math.round(palette[bi][1]);
    rgba[i * 4 + 2] = Math.round(palette[bi][2]);
    rgba[i * 4 + 3] = 255;
  }
...
function qualityPreset(opts) {
  const maxEdge = clamp(opts.maxEdge || 800, 200, 800);
  const colors = clamp(opts.colors == null ? 8 : Number(opts.colors), 2, 10);
  const fine = maxEdge >= 750 || colors >= 9;
  ...
  return {
    ...
    fine: fine,
    simple: simple,
    epsilon: opts.epsilon == null ? (fine ? 0.75 : simple ? 1.15 : 0.95) : Number(opts.epsilon),
    fitError: opts.fitError == null ? (fine ? 1.8 : simple ? 2.6 : 2.1) : Number(opts.fitError),
    cornerCos: fine ? 0.48 : 0.55,
    smoothPasses: fine ? 2 : 2,
    minPixFrac: fine ? 0.0006 : 0.0010,
  };
}
...
function destaircase(pts) {
  ...
  for (let pass = 0; pass < 12; pass++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const axisAB = (Math.abs(abx) < 0.65 && Math.abs(aby) >= 0.55) || (Math.abs(aby) < 0.65 && Math.abs(abx) >= 0.55);
      const axisBC = (Math.abs(bcx) < 0.65 && Math.abs(bcy) >= 0.55) || (Math.abs(bcy) < 0.65 && Math.abs(bcx) >= 0.55);
      const turned = (Math.abs(abx) < 0.65 && Math.abs(bcy) < 0.65) || (Math.abs(aby) < 0.65 && Math.abs(bcx) < 0.65);
      const short = Math.hypot(abx, aby) <= 3.6 && Math.hypot(bcx, bcy) <= 3.6;
...
function chaikinPreserveCorners(pts, rounds) {
  let ring = (pts || []).map(function (p) { return { x: p.x, y: p.y }; });
  for (let r = 0; r < (rounds || 1); r++) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      if ((v1x * v2x + v1y * v2y) / (l1 * l2) < 0.35) sharp[i] = 1;
    }
    const out = [];
    for (let i = 0; i < n; i++) {
...
function laplacianSmooth(pts, iters) {
  let ring = pts.map(function (p) { return { x: p.x, y: p.y }; });
  for (let n = 0; n < (iters || 2); n++) {
    ...
    for (let i = 0; i < m; i++) {
      ...
      const pt = ring[i];
      out[i] = {
        x: pt.x + 0.45 * ((prev.x + next.x) * 0.5 - pt.x),
        y: pt.y + 0.45 * ((prev.y + next.y) * 0.5 - pt.y),
      };
    }
    ring = out;
  }
...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  const scaled = majorityDownsample(img0.rgba, img0.width, img0.height, maxEdge);
  const w = scaled.w, h = scaled.h;
  const rgba = Buffer.from(scaled.rgba);
  ...
  // Posterize like a shop tracer: ignore AA fringe, pick real solids, snap every pixel.
  ...
  if (!palette.length) {
    ...
    palette = hierarchicalLabPalette(pixels, nColors, null);
  }
  if (!palette.length) return rectangleLayers(widthInN, heightInN, "#111111", "Fill");

  const snapped = snapToPalette(rgba, w, h, palette);
  let assign = snapped.assign;
  let palLabs = snapped.palLabs;
  let counts = snapped.counts;
  majoritySmooth(assign, w, h, q.smoothPasses + 2);
  morphOpenClose(assign, w, h);
  knockoutPaper(assign, palette, w, h);
  majoritySmooth(assign, w, h, 2);
  const minPix = Math.max(highQ ? 8 : 10, Math.floor(opaque * q.minPixFrac));
  ...
  mergeTinyAreas(assign, palette, palLabs, counts, minPix, 4 * 4);
  despeckleAssign(assign, w, h, minPix);
  counts = new Array(palette.length).fill(0);
  for (let i = 0; i < assign.length; i++) if (assign[i] >= 0) counts[assign[i]]++;
  ...
  for (let si = 0; si < specs.length; si++) {
    ...
    for (let t = 0; t < contours.length; t++) {
      ...
      pts = pts.map(function (p) { return { x: p.x + 0.5, y: p.y + 0.5 }; });
      pts = destaircase(pts);
      pts = simplifyClosed(pts, Math.max(1.25, dpEps));
      if (!pts || pts.length < 3) continue;
      pts = chaikinPreserveCorners(pts, 1);
  ...
  return { widthIn: widthInN, heightIn: heightInN, layers: annotateLayers(layers) };
}
...
function svgFromLayers(layers, widthIn, heightIn) {
  ...
  const groups = (layers || []).map((L) => {
    ...
    const seal = Math.max(w, h) * 0.0018;
    return "  <g fill=\"" + esc(L.hex) + "\" stroke=\"" + esc(L.hex) + "\" stroke-width=\"" + seal + "\" stroke-linejoin=\"round\" data-name=\"" + esc(L.nameGuess || "") + "\">\n" +
