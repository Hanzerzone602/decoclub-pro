...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  const dpEps = opts.epsilon == null ? (highQ ? 0.55 : 0.85) : opts.epsilon;
  const fitErr = opts.fitError == null ? (highQ ? 0.65 : 0.85) : opts.fitError;
  const layers = [];
  for (let c = 0; c < palette.length; c++) {
