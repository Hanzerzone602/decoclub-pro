...
function qualityPreset(opts) {
  ...
  return {
    ...
    fine: fine,
    simple: simple,
    epsilon: opts.epsilon == null ? (fine ? 0.95 : simple ? 1.25 : 1.1) : Number(opts.epsilon),
    fitError: opts.fitError == null ? (fine ? 3.1 : simple ? 4.2 : 3.5) : Number(opts.fitError),
    cornerCos: fine ? 0.48 : 0.55,
    smoothPasses: fine ? 2 : 2,
    minPixFrac: fine ? 0.0012 : 0.0018,
  };
}
...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  const minPix = Math.max(highQ ? 14 : 12, Math.floor(opaque * q.minPixFrac));
  ...
  for (let si = 0; si < specs.length; si++) {
    ...
    for (let t = 0; t < contours.length; t++) {
      ...
      pts = destaircase(pts);
      pts = simplifyClosed(pts, Math.max(2.6, dpEps + 1.2));
      if (!pts || pts.length < 3) continue;
      pts = chaikinPreserveCorners(pts, 2);
