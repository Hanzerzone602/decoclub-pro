...
function qualityPreset(opts) {
  ...
  return {
    ...
    fine: fine,
    simple: simple,
    epsilon: opts.epsilon == null ? (fine ? 0.75 : simple ? 1.1 : 0.9) : Number(opts.epsilon),
    fitError: opts.fitError == null ? (fine ? 0.7 : simple ? 1.1 : 0.85) : Number(opts.fitError),
...
function destaircase(pts) {
  ...
  for (let pass = 0; pass < 4; pass++) {
    ...
    for (let i = 1; i < ring.length - 1; i++) {
      ...
      const stair = (Math.abs(abx) < 1.15 && Math.abs(aby) > 0.35 && Math.abs(bcy) < 1.15 && Math.abs(bcx) > 0.35) ||
        (Math.abs(aby) < 1.15 && Math.abs(abx) > 0.35 && Math.abs(bcx) < 1.15 && Math.abs(bcy) > 0.35);
      if (stair) continue;
      out.push(b);
    }
...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  for (let si = 0; si < specs.length; si++) {
    ...
    for (let t = 0; t < contours.length; t++) {
      ...
      pts = destaircase(pts);
      pts = simplifyClosed(pts, Math.max(0.85, dpEps));
      pts = optimalPolygon(pts, Math.max(0.9, dpEps));
      if (!pts || pts.length < 3) pts = simplifyClosed(contours[t].pts, dpEps);
      if (!pts || pts.length < 3) continue;
      pts = laplacianSmooth(pts, 1);
