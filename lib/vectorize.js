...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  const nColors = clamp(opts.colors == null ? 8 : Number(opts.colors), 2, 12);
  const pixels = [];
  for (let i = 0; i < w * h; i++) {
    const a = rgba[i * 4 + 3];
    if (a < 12) continue;
  ...
  const palette = medianCut(pixels.map((p) => [p[0], p[1], p[2]]), nColors);
  ...
  const minPix = Math.max(8, Math.floor(pixels.length * 0.002));
  ...
  for (let c = 0; c < palette.length; c++) {
    ...
    for (let t = 0; t < contours.length; t++) {
      ...
      pts = simplifyClosed(pts, opts.epsilon == null ? 1.15 : opts.epsilon);
