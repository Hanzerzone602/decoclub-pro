...
function kmeansPlus(pixels, k, iters) {
  k = clamp(k || 8, 2, 16);
  if (!pixels.length) return [];
  const unique = new Map();
  for (let i = 0; i < pixels.length; i++) {
  ...
  const centers = [];
  centers.push([sample[0][0], sample[0][1], sample[0][2]]);
  ...
  for (let c = 1; c < k; c++) {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      for (let j = 0; j < centers.length; j++) {
        const d = dist2(sample[i], centers[j]);
        if (d < best) best = d;
      }
      d2[i] = best;
      sum += best;
  ...
  for (let iter = 0; iter < (iters || 10); iter++) {
    ...
    for (let i = 0; i < sample.length; i++) {
      ...
      sums[bi][0] += sample[i][0];
      sums[bi][1] += sample[i][1];
      sums[bi][2] += sample[i][2];
      sums[bi][3]++;
    }
    let moved = false;
    for (let c = 0; c < centers.length; c++) {
      ...
      if (Math.abs(nr - centers[c][0]) + Math.abs(ng - centers[c][1]) + Math.abs(nb - centers[c][2]) > 0.4) moved = true;
      centers[c] = [nr, ng, nb];
    }
    if (!moved) break;
  }
  return mergeNear(centers, 12 * 12);
}
...
function fitCubicPath(pts, scaleX, scaleY) {
  ...
  if (ring.length < 3) {
    return "M " + fmt(ring[0].x * scaleX) + " " + fmt(ring[0].y * scaleY) +
      (ring[1] ? " L " + fmt(ring[1].x * scaleX) + " " + fmt(ring[1].y * scaleY) : "") + " Z";
  }
  if (ring.length < 4) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    const next = ring[(i + 2) % n];
    const dx = p3.x - p0.x, dy = p3.y - p0.y;
    const seg = Math.hypot(dx, dy) || 1;
    const chord = { x: dx / seg, y: dy / seg };
    const inDir = dir(prev, p0);
    const outDir = dir(p3, next);
    const sharp0 = inDir.x * chord.x + inDir.y * chord.y < 0.72;
    const sharp1 = chord.x * outDir.x + chord.y * outDir.y < 0.72;
    const t0 = sharp0 ? chord : tangentAt(ring, i);
    const t1 = sharp1 ? chord : tangentAt(ring, (i + 1) % n);
    const k = seg / 3;
    d += " C " + fmt((p0.x + t0.x * k) * scaleX) + " " + fmt((p0.y + t0.y * k) * scaleY) +
      " " + fmt((p3.x - t1.x * k) * scaleX) + " " + fmt((p3.y - t1.y * k) * scaleY) +
      " " + fmt(p3.x * scaleX) + " " + fmt(p3.y * scaleY);
  }
  return d + " Z";
}
...
function vectorize(buf, widthIn, heightIn, opts) {
  ...
  const scaled = downsampleRgba(img0.rgba, img0.width, img0.height, opts.maxEdge || 900);
  ...
  for (let i = 0; i < w * h; i++) {
    const a = rgba[i * 4 + 3];
    if (a < 12) continue;
    pixels.push([rgba[i * 4], rgba[i * 4 + 1], rgba[i * 4 + 2], i]);
  }
  if (!pixels.length) {
    return rectangleLayers(widthInN, heightInN, "#111111", "Fill");
  }
  const palette = kmeansPlus(pixels.map((p) => [p[0], p[1], p[2]]), nColors, 10);
  ...
  const minPix = Math.max(12, Math.floor(pixels.length * 0.0015));
  ...
  for (let c = 0; c < palette.length; c++) {
    ...
    for (let t = 0; t < contours.length; t++) {
      ...
      pts = simplifyClosed(pts, opts.epsilon == null ? 0.85 : opts.epsilon);
      if (pts.length < 3) pts = contours[t].pts;
      const d = fitCubicPath(pts, sx, sy);
