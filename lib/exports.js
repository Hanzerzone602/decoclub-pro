...
const { vectorize, svgFromLayers, jobVectorOrTrace } = require("./vectorize");
const { epsFromLayers } = require("./eps");
const { digitizeJob } = require("./digitize");
const { stonesForJob } = require("./stones");
const { layersFromSvg, hasRealPaths } = require("./stitch/svgLayers");
...
/** Resolve absolute path to job.vector_svg under uploadsDir (or absolute path). */
function resolveVectorAbs(job, uploadsDir) {
  if (!job || !job.vector_svg) return null;
  try {
    const base = path.basename(job.vector_svg);
    if (uploadsDir) {
      const under = path.join(uploadsDir, base);
      if (fs.existsSync(under)) return under;
    }
    if (path.isAbsolute(job.vector_svg) && fs.existsSync(job.vector_svg)) return job.vector_svg;
    if (fs.existsSync(job.vector_svg)) return path.resolve(job.vector_svg);
  } catch (e) { /* ignore */ }
  return null;
}

function stripSvgShell(svgText) {
  let s = String(svgText || "").replace(/<\?xml[^?]*\?>\s*/i, "").trim();
  const m = s.match(/<svg\b([^>]*)>/i);
  if (!m) return null;
  const close = s.lastIndexOf("</svg>");
  if (close < 0) return null;
  return { attrs: m[1] || "", inner: s.slice(m.index + m[0].length, close).trim() };
}

function parseSvgDims(attrs, fallbackW, fallbackH) {
  let w = Number(fallbackW) || 1;
  let h = Number(fallbackH) || 1;
  const vb = String(attrs || "").match(
    /viewBox\s*=\s*["']\s*([-\d.eE+]+)\s+([-\d.eE+]+)\s+([-\d.eE+]+)\s+([-\d.eE+]+)\s*["']/
  );
  if (vb) {
    const vw = Math.abs(Number(vb[3]));
    const vh = Math.abs(Number(vb[4]));
    if (vw > 0) w = vw;
    if (vh > 0) h = vh;
  } else {
    const wm = String(attrs || "").match(/\bwidth\s*=\s*["']\s*([\d.]+)\s*(in|px|pt|mm|cm)?\s*["']/i);
    const hm = String(attrs || "").match(/\bheight\s*=\s*["']\s*([\d.]+)\s*(in|px|pt|mm|cm)?\s*["']/i);
    if (wm && Number(wm[1]) > 0) w = Number(wm[1]);
    if (hm && Number(hm[1]) > 0) h = Number(hm[1]);
  }
  return { width: Math.max(1e-6, w), height: Math.max(1e-6, h) };
}

/**
 * If job.vector_svg exists on disk, return inner markup (paths/groups only).
 * Strips XML declaration and outer <svg>...</svg>. Else null.
 */
function loadVectorInner(job, uploadsDir) {
  const abs = resolveVectorAbs(job, uploadsDir);
  if (!abs) return null;
  try {
    const shell = stripSvgShell(fs.readFileSync(abs, "utf8"));
    return shell && shell.inner ? shell.inner : null;
  } catch (e) {
    return null;
  }
}
...
/**
 * Place artwork into a sheet cell as editable vectors.
 * Prefer vector_svg wrapped in translate+scale; never emit <image href="/uploads/...">.
 */
function artForSheet(job, uploadsDir, x, y, w, h) {
  const abs = resolveVectorAbs(job, uploadsDir);
  if (abs) {
    try {
      const shell = stripSvgShell(fs.readFileSync(abs, "utf8"));
      if (shell && shell.inner) {
        const dims = parseSvgDims(shell.attrs, w, h);
        const sx = w / dims.width;
        const sy = h / dims.height;
        return `<g transform="translate(${x},${y}) scale(${sx},${sy})">${shell.inner}</g>`;
      }
    } catch (e) { /* fall through */ }
  }
  return `<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="none" stroke="#888" stroke-width="0.02"/>`;
}
...
function laserSvg(job, uploadsDir) {
  const c = jobContour(job, uploadsDir, 0);
  const artNote = artForSheet(job, uploadsDir, 0, 0, c.widthIn, c.heightIn);
  return svgFromContour(c, {
    stroke: "#111",
    fill: "none",
    strokeWidth: 0.02,
...
function gangSheetSvg(job, uploadsDir) {
  ...
  for (let i = 0; i < L.qty; i++) {
    ...
    const img = artForSheet(job, uploadsDir, x, y, L.artW, L.artH);
...
function stickerCutlineSvg(job, uploadsDir) {
  ...
  const ow = w + pad * 2;
  const oh = h + pad * 2;
  const art = artForSheet(job, uploadsDir, 0, 0, w, h);
...
function methodNotes(job) {
  const m = job.method;
  if (m === "embroidery") return "Machine files: design.dst (Tajima) + design.exp (Melco) + stitch-preview.svg. Hoop to size.";
  if (m === "rhinestone") return "Hotfix layout: stones.svg + stones.csv + stones.plt. SS map is hex-packed on the art mask.";
  if (m === "uv" || m === "uvdtf") return "Print UV/UV-DTF, gang if qty > 1, laminate if required. Contour for kiss-cut. Vectors: art.svg + art.eps.";
  if (m === "sublimation") return "Mirror art, 400F press typical. Tumbler wrap mockup is placement only. Vectors: art.svg + art.eps.";
  if (m === "vinyl") return "Cut from art.svg + art.eps. Weeding box, transfer tape. Watch mirrored heat-transfer.";
  if (m === "sign") return "art.svg + art.eps + cutter marks. Substrate notes in job.";
  if (m === "laser") return "SVG path + HPGL PLT plus art.svg/art.eps. Power/speed set on the machine.";
  if (m === "dtf") return "22in gang sheet, 0.125in gap. Powder and cure per film. Vectors: art.svg + art.eps.";
  return "Packet includes art.svg + art.eps vectors plus process files for this method.";
}
...
/**
 * Geometry for EPS / layer rebuilds: prefer job.vector.layers when they have
 * real path `d` data; otherwise parse job.vector_svg (vai-trace / invent-warp
 * often store empty paths[] while the SVG file keeps the curves).
 */
function layersForExport(job, vec, uploadsDir) {
  const widthIn = (vec && vec.widthIn) || (job && job.width_in) || 1;
  const heightIn = (vec && vec.heightIn) || (job && job.height_in) || 1;
  if (vec && hasRealPaths(vec)) {
    return { layers: vec.layers, widthIn: widthIn, heightIn: heightIn };
  }
  const abs = resolveVectorAbs(job, uploadsDir);
  if (abs) {
    try {
      const parsed = layersFromSvg(fs.readFileSync(abs, "utf8"), widthIn, heightIn);
      if (hasRealPaths(parsed)) {
        return {
...
function writeExports(job, uploadsDir, exportDir) {
  ...
  const buf = loadArtworkBuffer(job, uploadsDir);
  const vec = jobVectorOrTrace(job, buf);
  const geo = layersForExport(job, vec, uploadsDir);
  let artSvg = svgFromLayers(geo.layers, geo.widthIn, geo.heightIn);
  let artEps = epsFromLayers(geo.layers, geo.widthIn, geo.heightIn);
  if (job.vector_svg) {
    try {
      const abs = path.join(uploadsDir, path.basename(job.vector_svg));
      if (fs.existsSync(abs)) artSvg = fs.readFileSync(abs, "utf8");
    } catch (e) {}
  }
  if (job.vector_eps) {
    try {
      const abs = path.join(uploadsDir, path.basename(job.vector_eps));
      if (fs.existsSync(abs)) artEps = fs.readFileSync(abs);
    } catch (e) {}
  }
  let dstBuf = Buffer.alloc(0), expBuf = Buffer.alloc(0), stitchSvg = "", stitchCount = 0;
  try {
    const dig = digitizeJob(Object.assign({}, job, { vector: vec }), uploadsDir, {});
    dstBuf = dig.dst;
    expBuf = dig.exp;
    stitchSvg = dig.previewSvg;
    stitchCount = dig.stitchCount;
  } catch (e) { /* keep empty */ }
  let stonePack = { svg: "", csv: "x_in,y_in,ss,mm,color_name,hex\n", plt: "IN;\nSP0;\n", count: 0, stones: [] };
  try {
    stonePack = stonesForJob(Object.assign({}, job, { vector: vec }), uploadsDir, { ss: job.stone_ss || "SS10" });
  } catch (e) { /* keep empty */ }
  const files = {
    packet_json: "packet.json",
    art_svg: "art.svg",
    art_eps: "art.eps",
    design_dst: "design.dst",
    design_exp: "design.exp",
    stitch_preview_svg: "stitch-preview.svg",
    stones_svg: "stones.svg",
    stones_csv: "stones.csv",
    stones_plt: "stones.plt",
  ...
  const contents = {
    ...
    "design.dst": dstBuf,
    "design.exp": expBuf,
    "stitch-preview.svg": stitchSvg,
    "stones.svg": stonePack.svg,
    "stones.csv": stonePack.csv,
    "stones.plt": stonePack.plt,
    "cut-contour.svg": cutContourSvg(job, uploadsDir),
    "laser.svg": laserSvg(job, uploadsDir),
    "laser.plt": laserPlt(job, uploadsDir),
    "gang-sheet.svg": gangSheetSvg(job, uploadsDir),
  ...
  packet.stitchCount = stitchCount;
  packet.stoneCount = stonePack.count;
