...
function detectCorners(ring, cosThresh) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (vdot(v1, v2) < cosThresh) corners.push(i);
  }
  return corners;
}
...
function fitCubicPath(pts, scaleX, scaleY, opts) {
  ...
  const cornerCos = opts.cornerCos == null ? 0.55 : opts.cornerCos;
  ...
  const splits = corners.length ? corners.slice() : [];
  if (!corners.length) {
    // organic / circular: prefer 4 G1 segments for stable smooth closed fit
    const nSeg = ring.length >= 16 ? 4 : 2;
    for (let k = 0; k < nSeg; k++) splits.push(Math.floor((k * ring.length) / nSeg));
  }
