...
function sealSvgPaths(svgText) {
  ...
  const sw = Math.max(1.0, Math.min(pxW, pxH) * 0.0024);
...
function buildArgs(inPng, outSvg, opts) {
  ...
  /* Proven logo defaults: poster + cutout (Tiger Test winner). */
  args.push("--preset", String(opts.preset || "poster"));
  args.push(
    "--colormode", "color",
    "--hierarchical", opts.hierarchical || "cutout",
    "--mode", "spline",
    "--filter_speckle", String(opts.filterSpeckle != null ? opts.filterSpeckle : (colors <= 4 ? 14 : colors <= 8 ? 12 : 4)),
  ...
  if (opts.cornerThreshold != null) args.push("--corner_threshold", String(opts.cornerThreshold));
  if (opts.segmentLength != null) args.push("--segment_length", String(opts.segmentLength));
  if (opts.spliceThreshold != null) args.push("--splice_threshold", String(opts.spliceThreshold));
...
function vectorizeBuffer(pngBuf, opts) {
  ...
  try {
    let workBuf = pngBuf;
    /* Preprocess (posterize/upscale) is opt-in — default off; it was hurting logo quality. */
    if (opts.preprocess) {
