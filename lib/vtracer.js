...
function sealSvgPaths(svgText) {
  ...
  const sw = Math.max(0.25, Math.min(pxW, pxH) * 0.00045);
...
function buildArgs(inPng, outSvg, opts) {
  ...
  /* Best fidelity: no poster preset, stacked splines (matches raw Tiger score ~4.7). */
  if (opts.preset && opts.preset !== false && opts.preset !== "none") {
    args.push("--preset", String(opts.preset));
  }
  const speckle = opts.filterSpeckle != null ? opts.filterSpeckle
    : (colors <= 4 ? 6 : colors <= 8 ? 4 : 2);
  const precision = opts.colorPrecision != null ? opts.colorPrecision
    : (colors <= 4 ? 5 : colors <= 8 ? 6 : 7);
  args.push(
    "--colormode", "color",
    ...
    "--path_precision", String(opts.pathPrecision != null ? opts.pathPrecision : (Number(opts.colors) <= 8 ? 2 : 3))
  );
  if (opts.gradientStep != null) args.push("--gradient_step", String(opts.gradientStep));
  return args;
}
...
function vectorizeBuffer(pngBuf, opts) {
  ...
  try {
    ...
    if (opts.seal) svg = sealSvgPaths(svg);
