...
function sealSvgPaths(svgText, sealOpts) {
  sealOpts = sealOpts || {};
  ...
  const srcMin = sealOpts.srcMinEdge != null ? Number(sealOpts.srcMinEdge) : Math.min(pxW, pxH);
  const small = sealOpts.small === true || srcMin < 700;
  const sw = sealOpts.strokeWidth != null
    ? Number(sealOpts.strokeWidth)
    : (small
      ? Math.max(0.70, srcMin * 0.0025)
      : Math.max(0.45, srcMin * 0.00055));
...
/* Stacked VTracer paints full-canvas base colors (e.g. tiger gold) under detail.
   Stripping them punches holes; stripping only paper leaves an orange square.
   Keep all paths — paper is nearly invisible on the studio light bg. */
...
function bilinearUpscale(rgba, width, height, scale) {
  const s = scale;
  if (!s || s <= 1) return { rgba: rgba, width: width, height: height };
  ...
  for (let y = 0; y < nh; y++) {
    ...
    for (let x = 0; x < nw; x++) {
      ...
      for (let c = 0; c < 4; c++) {
        ...
        const v0 = rgba[i00] * (1 - wx) + rgba[i10] * wx;
        const v1 = rgba[i01] * (1 - wx) + rgba[i11] * wx;
...
function softScalePng(pngBuf) {
  const decoded = decodePng(pngBuf);
  const maxEdge = Math.max(decoded.width, decoded.height);
  if (maxEdge >= 700) return pngBuf;
  /* Bilinear stand-in for Lanczos — 3× under 450, 2× under 700 (bakeoff winner). */
  const scale = maxEdge < 450 ? 3 : 2;
  const up = bilinearUpscale(decoded.rgba, decoded.width, decoded.height, scale);
...
function buildArgs(inPng, outSvg, opts) {
  ...
  /* Adaptive stacked spline (New Bot bakeoff winner). Never use poster. */
  if (opts.preset && opts.preset !== false && opts.preset !== "none") {
    args.push("--preset", String(opts.preset));
  }
  const maxEdge = Number(opts.srcMaxEdge) || 0;
  const small = opts.small === true || (maxEdge > 0 && maxEdge < 700);
  let speckle, precision, corner, segment, splice, pathPrec, grad;
  if (small) {
    speckle = opts.filterSpeckle != null ? opts.filterSpeckle : (colors <= 4 ? 4 : 2);
    precision = opts.colorPrecision != null ? opts.colorPrecision : (colors <= 4 ? 6 : 8);
    corner = opts.cornerThreshold != null ? opts.cornerThreshold : 50;
    segment = opts.segmentLength != null ? opts.segmentLength : 3.5;
    splice = opts.spliceThreshold != null ? opts.spliceThreshold : 40;
    pathPrec = opts.pathPrecision != null ? opts.pathPrecision : 3;
    grad = opts.gradientStep != null ? opts.gradientStep : 16;
  } else {
    /* Locked large baseline — no seal path; keep tiger-test ~4.46. */
    speckle = opts.filterSpeckle != null ? opts.filterSpeckle : (colors <= 4 ? 6 : colors <= 8 ? 4 : 2);
    precision = opts.colorPrecision != null ? opts.colorPrecision : (colors <= 4 ? 5 : colors <= 8 ? 6 : 7);
    corner = opts.cornerThreshold != null ? opts.cornerThreshold : 60;
    segment = opts.segmentLength != null ? opts.segmentLength : 4;
    splice = opts.spliceThreshold != null ? opts.spliceThreshold : 45;
    pathPrec = opts.pathPrecision != null ? opts.pathPrecision : (colors <= 8 ? 2 : 3);
    grad = opts.gradientStep;
  }
  args.push(
    "--colormode", "color",
    "--hierarchical", opts.hierarchical || "stacked",
    "--mode", "spline",
    "--filter_speckle", String(speckle),
    "--color_precision", String(precision),
    "--corner_threshold", String(corner),
    "--segment_length", String(segment),
    "--splice_threshold", String(splice),
    "--path_precision", String(pathPrec)
  );
  if (grad != null) args.push("--gradient_step", String(grad));
  return args;
}
...
function vectorizeBuffer(pngBuf, opts) {
  ...
  try {
    ...
    let srcW = 0, srcH = 0;
    try {
      const dec0 = decodePng(pngBuf);
      srcW = dec0.width; srcH = dec0.height;
    } catch (e) { /* keep 0 */ }
    const srcMax = Math.max(srcW, srcH);
    const srcMin = Math.min(srcW || 1000, srcH || 1000);
    const small = srcMax > 0 && srcMax < 700;
    opts = Object.assign({}, opts, { srcMaxEdge: srcMax, srcMinEdge: srcMin, small: small });
    if (opts.preprocess) {
      try { workBuf = preprocessPng(pngBuf, opts); } catch (e) { workBuf = pngBuf; }
    } else if (opts.softScale === true || (opts.softScale !== false && small)) {
      /* Soft upscale for small logos only (bakeoff winner). Large stays 1:1. */
      try { workBuf = softScalePng(pngBuf); } catch (e) { workBuf = pngBuf; }
    }
    ...
    /* Do NOT remap SVG fills to the cluster palette — that posterizes and ruins curves.
       Keep VTracer geometry/colors; build recolor layers from clustered representatives. */
    /* Light seal on small inputs only — large front keeps locked no-seal baseline. */
    if (opts.seal === true || (opts.seal !== false && small)) {
      svg = sealSvgPaths(svg, { srcMinEdge: srcMin, small: small });
    }
    const vec = parseSvg(svg, opts.widthIn, opts.heightIn);
    /* Collapse layer list to ~maxColors for UI by merging near fills */
    if (vec.layers && vec.layers.length > maxColors) {
      const byRep = new Map();
      vec.layers.forEach(function (L) {
        const rep = clustered.map[normalizeHex(L.hex)] || normalizeHex(L.hex);
        if (!byRep.has(rep)) {
          byRep.set(rep, Object.assign({}, L, { hex: rep, paths: (L.paths || []).slice() }));
        } else {
          const cur = byRep.get(rep);
          cur.paths = (cur.paths || []).concat(L.paths || []);
        }
      });
      vec.layers = annotateLayers(Array.from(byRep.values()));
    } else {
      vec.layers = annotateLayers(vec.layers);
    }
    let eps = null;
    try {
