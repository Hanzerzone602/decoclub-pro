...
/**
 * Print-area presets on real flat-front blank photos (S&S-style).
 * Fractions of image width/height. Wearer's left chest = higher x (viewer right).
 * Scale % grows/shrinks about the preset center; offset % nudges.
 */
const PHOTO_PLACES = {
  chest: { x: 0.31, y: 0.24, w: 0.38, h: 0.32 },
  center: { x: 0.31, y: 0.24, w: 0.38, h: 0.32 },
  left_chest: { x: 0.54, y: 0.22, w: 0.18, h: 0.16 },
  full: { x: 0.26, y: 0.22, w: 0.48, h: 0.52 },
  front: { x: 0.26, y: 0.22, w: 0.48, h: 0.52 },
  // Back: until back-view blanks ship, same framing as full front on the flat photo
  back: { x: 0.28, y: 0.22, w: 0.44, h: 0.50 },
  // Youth: smaller print area, slightly higher on body (adult flat used until youth blanks ship)
  youth: { x: 0.34, y: 0.20, w: 0.32, h: 0.28 },
  youth_chest: { x: 0.34, y: 0.20, w: 0.32, h: 0.28 },
  wrap: { x: 0.26, y: 0.22, w: 0.48, h: 0.52 },
};
...
function photoArtBox(w, h, job) {
  ...
  const key = String((job && job.placement) || "chest").toLowerCase();
  const preset = PHOTO_PLACES[key] || PHOTO_PLACES.chest;
  const baseW = w * preset.w;
  const baseH = h * preset.h;
  const boxW = Math.round(baseW * scale);
  const boxH = Math.round(baseH * scale);
  const cx = w * (preset.x + preset.w / 2) + (offX / 100) * w;
  const cy = h * (preset.y + preset.h / 2) + (offY / 100) * h;
  return {
