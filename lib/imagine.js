...
function imagineConfigured() {
  // HARD LOCK 2026-09-17: paid SpaceXAI API — off unless David explicitly enables.
  if (String(process.env.DECOCLUB_IMAGINE || "").trim() !== "1") return false;
...
function generateImage(opts) {
  opts = opts || {};
  if (!imagineConfigured()) return Promise.reject(new Error("AI Generate is not configured"));
  return resolveImagineModel().then(function (model) {
    ...
    return post(payload).then(function (res) {
      ...
      if (res.status >= 400) {
        ...
        const msg = (errObj && (errObj.message || errObj)) || "AI Generate failed";
        throw new Error(typeof msg === "string" ? msg : "AI Generate failed");
      }
